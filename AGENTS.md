...
pnpm format               # prettier --write everything (⚠️ never on docs/index.html)
...
- **R-A07 — English ships, users translate.** No Persian (or other language) in the bundle. RTL via `dir="auto"`. AI output language (`preferredIntelligenceLanguage`) is independent of UI language.
- **R-A08 — Insights carry their sources; explainability is a feature.** Every generated insight stores references to the articles it came from, and the UI can show them (citations `[N]`, source lists). If a reader can't tell *why* an insight exists, the feature is incomplete — citations were silently dropped once (v1.0.2); don't regress.
...
- **R-D01 — Version changes MUST use `/version-sync`** — root `package.json` by hand → `pnpm version:sync` → README + docs/index.html + codenames by hand → rebuild `@vorynth/types` → `pnpm version:check`.
...
4. **Confirm** — if the change is risky (migration, architecture change, security, destructive), present the plan and get the user's go-ahead before modifying. Small reversible changes can proceed.
...
- **Release targets (CI-built, downloadable):** Windows x64 + ARM64, macOS Apple Silicon, Linux x86_64 + ARM64 (deb/rpm/AppImage), FreeBSD (native VM build).
- **Experimental:** HarmonyOS NEXT (ARM64 `.so` + frontend bundle via CI — raw DevEco Studio bundle, no `.hap` yet). macOS Intel: CI builds exist but no DMG is published — landing page says "support coming soon".
- Releases: draft GitHub Release on `v*` tag push (`tauri-apps/tauri-action`), English title/description.
- Landing page: `omidnw.github.io/vorynth/` from `docs/` (doc-only changes skip CI but still deploy Pages).
...
- Port: engine resolves `--port` → `PORT` env → fallback `34117`; frontend uses `FIXED_PORT = 34117` — dev-mode connection failures have happened when these drifted.
- Rate limiter: 5 LLM requests/min sliding window; `VORYNTH_LLM_RPM` env override — long jobs must respect it or they'll silently fail.
