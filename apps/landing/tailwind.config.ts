 ...
 *
 * Mirrors the desktop app's theme (apps/desktop/tailwind.config.ts) so the real
 * desktop components render identically here. The COLOR VALUES are not
 * duplicated — they read the same `--color-*` CSS variables from theme.css
 * (the single source of truth). Keep the extend below in sync with desktop.
 *
 * `content` scans BOTH source trees so every utility class the desktop
 * components use is generated in the landing build too.
 */
const config: Config = {
	darkMode: "class",
	content: [
