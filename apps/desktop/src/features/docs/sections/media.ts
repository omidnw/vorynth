...
export const mediaSection: DocsSection = {
	...
	blocks: [
		...
		},
		{
			type: "features",
			items: [
				{ icon: "today", label: "Relationship to the Brief", text: "Media belongs to articles, and articles appear in the Brief as collected stories — the media dashboard shows the ones you deliberately kept." },
				{ icon: "inventory_2", label: "Relationship to the Archive", text: "The same articles live in the Archive; keeping media local means it stays available even if the source site changes or goes away — but it's a per-item, deliberate choice, never automatic." },
			],
		},
	],
};
