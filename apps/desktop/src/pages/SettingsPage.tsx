	...
	return (
		<GhostCard>
			...
			<Toggle
				...
				onChange={(v) =>
					toggleMode.mutate(v ? "intelligence" : "news")
				}
			/>
		</GhostCard>
	);
...
function LlmProviderSection({
		...
		const confirmDeleteEnabled =
			(appSettings?.["ui.confirmDeleteProvider"] as boolean | undefined) ??
			true;
				...
				{configured ? (
					...
					<div className="mb-4 flex items-center gap-3 border-l-2 border-warning bg-surface-container-low px-4 py-3 rounded">
						...
						<p className="font-body text-body-sm text-on-surface-variant">
							<strong className="text-on-surface">Intelligence mode is on,</strong>{" "}
							...
							<div className="flex items-start gap-3">
								...
								<div className="flex-1">
									...
									<p className="font-body text-body-sm text-on-surface-variant mb-3">
										The provider configuration and encrypted key will be
...
function ReCollectButton() {
	...
	return (
		<>
			<Button variant="secondary" icon="sync_problem" onClick={() => setShowConfirm(true)}>
...
function RegenerateInsightsButton() {
	...
	return (
		<>
			<Button variant="secondary" icon="auto_awesome" onClick={() => setShowConfirm(true)}>
...
/** Button for the "Translate Story Titles" card. */
function TranslateTitlesButton() {
	const { t } = useTranslation();
	const isActive = useJobsStore((s) => s.isActive("regenerate"));
	const startJob = useJobsStore((s) => s.startTranslateTitles);
	const [showConfirm, setShowConfirm] = useState(false);
	...
	return (
		<>
			<Button variant="secondary" icon="translate" onClick={() => setShowConfirm(true)}>
